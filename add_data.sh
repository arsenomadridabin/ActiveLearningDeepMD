...
if [ "$new_number" -gt 1 ]; then
    ...
    if [ -d "$prev_folder" ]; then
        echo "Copying top-level set.* folders from $prev_folder" >> log
        
        for subdir in train validate; do
            if [ -d "$prev_folder/$subdir" ]; then
                mkdir -p "$new_folder/$subdir"
                find "$prev_folder/$subdir" -maxdepth 1 -type d -name "set.*" -exec bash -c 'copy_folder_sequential "$0" "'$new_folder/$subdir'"' {} \;
            fi
        done
    else
